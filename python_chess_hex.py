...
class ChessEngine:
    ...
    def notation_to_board(self, nearest_loc):
        ...
        for key in all_notation_dicts[0:11]:
            if nearest_loc in key[
                0].values():  # узнаем в каком массиве из матрицы ближ шестиугольник к клику (номер столбца)
                column += i
                for key, value in key[
                    0].items():  # узнаем ключ эл-ма по значению, ключ в этом случае - номер по списку (номер строки)
